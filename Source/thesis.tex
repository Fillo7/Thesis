...
and an example of its use. Frameworks which are not publicly available or focus only on a specific subset of computations are not discussed here. The
final section of this chapter includes motivation for development of KTT framework.

\section{CLTune}
Todo...

\subsection{Autotuning}
In context of compute APIs, the autotuning process usually involves adding parameters affecting performance to kernel source code and finding the
optimal combination of these parameters. Figure \ref{constant_memory} contains a simple example of tuning parameter usage. The process in its basic
form consists of the following main steps:
\begin{enumerate}
    \item Definition of tuning parameters and their possible values, modification of kernel source file to support these parameters. This step is done
    by a program developer.
    \item Generating of configuration space, each configuration contains unique combination of parameters.
    \item Space traversal, launching of kernel in each tested configuration and measurement of execution duration. Total number and order of tested
    configurations depends on the utilized search method (e.g., full search, random search, utilization of heuristics).
