...
in picture \ref{opencl-hierarchy}.
...
In OpenCL, the NDRange size in a given dimension is specified as size of a work-group multiplied by number of work-groups. However, in CUDA the grid
...
\hline
Supported APIs & CUDA, OpenCL & CUDA, OpenCL & any language or API \\
Tuning parameters & kernel code only & kernel and host code & kernel and host code \\
Parameter constraints & \multicolumn{1}{c}{\checkmark} & \multicolumn{1}{c}{\checkmark} & requires additional user effort \\
...
Kernel compositions & \multicolumn{1}{c}{X} & \multicolumn{1}{c}{X} & requires additional user effort \\
...
\label{ktt-structure}
KTT structure is loosely based on layered architecture pattern, which is often employed for developing enterprise applications. Individual functionality
is split into multiple components across several layers. On lower layers, each component is independent and focuses on a single task (e.g., managing of
kernels), which makes it easier to extend existing functionality and introduce new features. Functionality of individual components is interconnected
on higher layers with the topmost layer serving as public API. Figure \ref{ktt-class-diagram} contains high-level class diagram of KTT framework, which
includes all of the major components and dependencies between them.
...
\begin{itemize}
    ...
    \item TunerCore
    \item KernelManager
    \item ArgumentManager
    \item TuningRunner
    \item KernelRunner
    \item ComputeEngine
    \item ConfigurationManager
    \item ResultValidator
    \item ManipulatorInterface
\end{itemize}

\section{Tuner}
Tuner component represents a major part of public API. Its methods were described in chapter \ref{ktt-api}. In order to hide internal framework
...
(i.e., TunerCore) and a pointer to its instance. However, the actual TunerCore header with method definitions is directly included only in Tuner
...
no need to include internal headers for components such as TunerCore, which makes upgrading to newer version of framework easier in certain situations
(e.g., if methods in TunerCore are modified in newer version of the framework, but methods in Tuner API remain the same, software which utilizes the
framework only needs to replace library file without recompilation).

\section{TunerCore}
TunerCore makes up the main internal part of the framework. It owns and manages several smaller components and exposes subset of their functionality
