...
copy / move constructors, destructor or assignment operators, which effectively makes it safe to copy with functions such as std::memcpy.}. However,
this is not much of a problem because data which is copied into e.g., GPU memory should be trivially copyable either way.
...
\begin{enumerate}
    ...
    \item Computation duration is reported back to configuration manager, the duration can be utilized by searcher to optimize exploration of configuration
    space. Durations for failed computations or invalid results are reported as infinity.
\end{enumerate}
...
tuner always launches computation with the best found configuration after the exploration of configuration space is finished.
